...
proc CopyAlertID { arg } {
    ...
    if { $ACTIVE_EVENT && !$MULTI_SELECT} {
        ...
        # Copy to clipboard
       clipboard clear
       clipboard append $event_id

    }
}

proc SplunkAlertID { arg } {
    ...
    if { $ACTIVE_EVENT && !$MULTI_SELECT} {
        ...
        set cnxID [lindex $sidcidList 1]
        set sensorID [lindex $sidcidList 0]
        set ipAddr1 [$CUR_SEL_PANE(name) getcells $selectedIndex,srcip]
        set ipAddr2 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstip]
        set ipPort1 [$CUR_SEL_PANE(name) getcells $selectedIndex,srcport]
        set ipPort2 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstport]
        set eventMsg [$CUR_SEL_PANE(name) getcells $selectedIndex,event]

        set starttime [clock scan "120 min ago" -base [clock scan [$CUR_SEL_PANE(name) getcells $selectedIndex,date] -timezone :UTC] -timezone :US/Pacific]
        set endtime [expr $starttime + 14400]
...
proc SplunkIPQuery { side } {
    ...
    if { $ACTIVE_EVENT && !$MULTI_SELECT} {
        ...
        if { $side == "srcip"} {
            ...
            set ipAddr1 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstip]
            set ipAddr2 "*"
        } else {
            set ipAddr1 [$CUR_SEL_PANE(name) getcells $selectedIndex,srcip]
            set ipAddr2 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstip]
...
proc SplunkPortQuery { arg } {
    ...
    if { $ACTIVE_EVENT && !$MULTI_SELECT} {
        ...
        if { $arg == "srcport" } {
            ...
            set ipPort1 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstport]
            set ipPort2 "*"
        } else {
            set ipPort1 [$CUR_SEL_PANE(name) getcells $selectedIndex,srcport]
            set ipPort2 [$CUR_SEL_PANE(name) getcells $selectedIndex,dstport]
